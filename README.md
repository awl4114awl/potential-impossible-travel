...
The investigation graph provides a visual representation of relationships between the user account, associated alerts, and connected infrastructure.

Analysis of the graph reveals that a single user account is the central entity, linked to multiple "Impossible Travel" alerts. These alerts are correlated and originate from repeated authentication activity within a short time window, rather than isolated events.

All activity is associated with a single external IP address (20.122.36.194), which was identified as the Azure virtual machine used during testing. The presence of a consistent IP and repeated alert pattern indicates controlled and predictable behavior rather than random or distributed access attempts.

The circular relationship between the user and IP highlights repeated sign-in cycles between two geographic locations, reinforcing the detection of impossible travel conditions.

Notably, no additional users, devices, or IP addresses were observed in the graph. There is no evidence of lateral movement, privilege escalation, or persistence mechanisms.

Overall, the investigation confirms that the alerts were generated by intentional lab activity simulating geographically disparate logins, with no indicators of malicious compromise.
...
- Repeated alerts observed due to overlapping query windows confirm detection reliability but highlight the need for tuning in production environments.
- Impossible travel detections alone are not definitive indicators of compromise and should be validated with supporting telemetry.
- Correlating identity signals with device, network, and authentication context is critical for accurate triage.
- Detection rules should balance sensitivity and noise reduction to remain operationally effective.
- This lab demonstrates validation of detection logic, not a real-world compromise scenario.
